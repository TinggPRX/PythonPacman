...
try:
    ...
    ghost_sound = pygame.mixer.Sound('assets/sounds/ghost.wav')
except:
    class DummySound:
        def play(self): pass
    eat_sound = death_sound = ghost_sound = DummySound()

def run_game():
    global WIDTH, HEIGHT, screen, timer, fps, font, level, color, player_images
    global blinky_img, pinky_img, inky_img, clyde_img, spooked_img, dead_img
    ...
    global clyde_x, clyde_y, clyde_direction, counter, flicker, turns_allowed
    global direction_command, player_speed, score, powerup, power_counter
    global eaten_ghost, targets, blinky_dead, inky_dead, clyde_dead, pinky_dead
    global blinky_box, inky_box, clyde_box, pinky_box, moving, ghost_speeds
    global startup_counter, lives, game_over, game_won
    global PI, current_level, high_score, paused
    ...
    # Khởi tạo lại các biến khác như trong code gốc
    player_images = []
    for i in range(1, 5):
    ...
    while run:
        ...
        if moving:
            ...
            if not powerup:
                if abs(player_x - blinky_x) < 30 and abs(player_y - blinky_y) < 30 and not blinky_dead:
                    lives -= 1
                    startup_counter = 0
                    player_x = 450
                    player_y = 663
                    ...
                    if lives <= 0:
                        game_over = True
                elif abs(player_x - inky_x) < 30 and abs(player_y - inky_y) < 30 and not inky_dead:
                    lives -= 1
                    startup_counter = 0
                    player_x = 450
                    player_y = 663
                    ...
                    if lives <= 0:
                        game_over = True
                elif abs(player_x - pinky_x) < 30 and abs(player_y - pinky_y) < 30 and not pinky_dead:
                    lives -= 1
                    startup_counter = 0
                    player_x = 450
                    player_y = 663
                    ...
                    if lives <= 0:
                        game_over = True
                elif abs(player_x - clyde_x) < 30 and abs(player_y - clyde_y) < 30 and not clyde_dead:
                    lives -= 1
                    startup_counter = 0
                    player_x = 450
                    player_y = 663
                ...
                if abs(player_x - blinky_x) < 30 and abs(player_y - blinky_y) < 30 and not blinky_dead and not eaten_ghost[0]:
                    blinky_dead = True
                    eaten_ghost[0] = True
                    score += 200
                if abs(player_x - inky_x) < 30 and abs(player_y - inky_y) < 30 and not inky_dead and not eaten_ghost[1]:
                    inky_dead = True
                    eaten_ghost[1] = True
                    score += 200
                if abs(player_x - pinky_x) < 30 and abs(player_y - pinky_y) < 30 and not pinky_dead and not eaten_ghost[2]:
                    pinky_dead = True
                    eaten_ghost[2] = True
                    score += 200
                if abs(player_x - clyde_x) < 30 and abs(player_y - clyde_y) < 30 and not clyde_dead and not eaten_ghost[3]:
                    clyde_dead = True
                    eaten_ghost[3] = True
                    score += 200

            if abs(player_x - blinky_x) < 30 and not powerup:
                death_sound.play()
        ...
        # Phần xử lý va chạm và sự kiện giữ nguyên như trong code gốc
        # ...
        
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
                return  # Thoát khỏi hàm run_game và trở về menu
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_DOWN:
                    direction_command = 3
                if event.key == pygame.K_ESCAPE:  # Thêm phím ESC để quay lại menu
                    run = False
                    return
                if event.key == pygame.K_SPACE and (game_over or game_won):
                    # Reset game khi nhấn space
                    powerup = False
                    power_counter = 0
                    lives = 3
                    startup_counter = 0
...
def draw_misc():
    ...
    screen.blit(high_score_text, (WIDTH//2 - high_score_text.get_width()//2, 15))
    ...
    # Hiển thị thời gian power-up còn lại
    if powerup:
        power_text = font.render(f'POWER: {30 - power_counter//20}', True, 'cyan')
        screen.blit(power_text, (WIDTH - 300, 15))
...
def draw_player():
    ...
    if startup_counter < 180 and not game_over and not game_won:
        # Nhấp nháy khi bị mất mạng
        if startup_counter // 10 % 2 == 0:
            if direction == 0:
